...
class PluginDescriptor:
    def __init__(self, metadata: PluginMetadata, plugin_factory: Callable[[], Any] | type[Any]) -> None:
...
def load_plugins_from_config_or_fail(
    config_path: Path, interpolation_vars: dict[str, str] | None = None
) -> list[LoadedPlugin]:
...
def _load_plugin_description_from_file_or_fail(path: Path) -> PluginDescription:
    ...
    if not hasattr(description, "get_plugin") or not callable(getattr(description, "get_plugin")):
        raise RuntimeError(f"register() must return an object with get_plugin(): {path}")
    if not hasattr(description, "get_metadata") or not callable(getattr(description, "get_metadata")):
        raise RuntimeError(f"register() must return an object with get_metadata(): {path}")
    return description
