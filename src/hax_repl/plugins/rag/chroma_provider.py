...
class ChromaVectorRagProvider(RagProvider):
    def __init__(self) -> None:
        ...
        if not api_key:
            raise RuntimeError(f"{OPENROUTER_KEY_ENV} is required for Chroma vector embeddings.")
    ...
    def _embed_many(self, texts: list[str]) -> list[list[float]]:
        ...
        for item in data:
            if not isinstance(item, dict):
                raise RuntimeError("OpenRouter embeddings response contains non-object item.")
            raw_embedding = item.get("embedding")
            if not isinstance(raw_embedding, list):
                raise RuntimeError("OpenRouter embeddings response item missing 'embedding' list.")
