...
def _build_repl_click_group() -> click.Group:
    ...
    def help_command(ctx: click.Context, topic: tuple[str, ...]) -> None:
        ...
        for name in topic:
            if not isinstance(target, click.Group):
                raise click.UsageError(f"Command {' '.join(topic)} has no sub-commands.", ctx=ctx)
            next_ctx = click.Context(target, info_name=target_info_name, parent=target_parent)
    ...
    @conversation.command(name="delete-last-message", help="Delete the most recent message in session history.")
    @click.pass_obj
    ...
    def conversation_generate_again(state: ReplCommandContext) -> None:
        ...
        try:
            with state.console.status("[yellow]thinking...[/yellow]", spinner="dots") as status:
    ...
    @history.command(name="append", help="Append text to session history as a user prompt.")
    ...
    def history_append(ctx: click.Context, text_tokens: tuple[str, ...]) -> None:
        ...
        state.console.print("[green]Appended to history as user prompt (not sent).[/green]")

    @repl_commands.group(help="Display helper commands.")
    def show() -> None:
        """Show hidden or derived information from prior responses."""

    @show.command(name="last-thinking", help="Show the hidden thinking block from the last response.")
    @click.pass_obj
    ...
    @copy.command(name="last-code", help="Copy the last fenced code block from the latest response.")
    @click.pass_obj
    def copy_last_code(state: ReplCommandContext) -> None:
        ...
        if not last_code:
            state.console.print("[yellow]No fenced code block found in last response.[/yellow]")
            return
        ok, message = _copy_to_clipboard(last_code)
        state.console.print(f"[green]{message}[/green]" if ok else f"[yellow]{message}[/yellow]")
    ...
    @repl_commands.command(name="file", help="Queue a file payload for inclusion in the next prompt.")
    ...
    @repl_commands.command(name="macro", help="Expand macro text and queue it for the next prompt.")
    @click.argument("text_tokens", nargs=-1)
    @click.pass_context
    ...
    def _print_loaded_functions(state: ReplCommandContext) -> None:
        ...
        for function in loaded:
            state.console.print(f"  - {function.name} (schema_hash={function.schema_hash})")
    ...
    @click.pass_context
    def functions_call(ctx: click.Context, name: str, json_tokens: tuple[str, ...]) -> None:
    ...
    def mcp_load(state: ReplCommandContext, descriptor_path: str) -> None:
        ...
        try:
            ...
            state.console.print(f"[bold red]Failed to load MCP descriptor:[/bold red] {exc}")
            return
    ...
    def mcp_call(
        ...
        if not json_tokens:
            raise click.UsageError("Usage: .mcp call <client-name> <tool-name> <json-args>", ctx=ctx)
        state = cast(ReplCommandContext, ctx.obj)
        arguments_json = " ".join(json_tokens).strip()
        try:
            result_json = state.runtime.invoke_mcp_tool(client_name, tool_name, arguments_json)
        except Exception as exc:
    ...
    @rag.command(name="update", help="Update an index from one or more source files/directories.")
    @click.argument("provider")
    ...
    def rag_update(
        ...
        if not sources:
            raise click.UsageError("Usage: .rag update <provider> <index> <path1> [path2 ...]", ctx=ctx)
        state = cast(ReplCommandContext, ctx.obj)
        try:
    ...
    def rag_query(
        ...
        if not query_tokens:
            raise click.UsageError("Usage: .rag query <provider> <index> <query>", ctx=ctx)
        state = cast(ReplCommandContext, ctx.obj)
        ...
        for chunk in result.chunks:
            ...
            state.console.print(textwrap.shorten(chunk.text.replace("\n", " "), width=220, placeholder=" ..."))
    ...
    @click.pass_context
    def agent_start(ctx: click.Context, agent_name: str, goal_tokens: tuple[str, ...]) -> None:
...
def _build_click_command_phrases(root: click.Group) -> list[str]:
    ...
    def _walk(group: click.Group, prefix: tuple[str, ...], parent_ctx: click.Context) -> None:
        for name in group.list_commands(parent_ctx):
            ...
            if isinstance(command, click.Group):
                child_ctx = click.Context(command, info_name=" ".join(parts), parent=parent_ctx)
...
def _handle_command(
    *,
    ...
    if not args:
        return True
    state = ReplCommandContext(runtime=runtime, console=console, pending_includes=pending_includes)
    try:
