...
- Python for main implementation and plugin system -- the tool is noticeably slower to start up compared to a single rust binary. Not exceedingly slow, but don't consider it a major detriment. 
- More verbose output in the repl -- thinking time, query character count etc. I'm paying per token in the API, I like to see the size of the chat. 
- No auto-summarization -- explicit command will suit better

```
# ❯ uv run hax-repl
HAX LLM REPL  (type .exit to quit)
Submit: Ctrl+J, Esc+Enter, or Ctrl+Enter (CSI-u terminals). New line: Enter. Complete: Tab.
2026-02-13T17:15:40.133428+00:00/default-agent|user)
QUERY [1]: test

RESULT [1]:
stats: model=anthropic/claude-sonnet-4.5 | session=2026-02-13T17:15:40.133428+00:00 | prompt_chars=4
Hello! I'm ready to help you test the available tools. 
done: total=6437 ms | first_token=6427 ms | thinking=0 ms | query_chars=4 | response_chars=792
2026-02-13T17:15:40.133428+00:00/default-agent|user)
QUERY [2]:
```
